...
def pressure_interpolate_reaction(reaction, pressure):
    ...
    # The pressure is between two available presssures points
        ...
        # If not we interpolate
            ...
            min_E = np.min([E1, E2])
            E_scaled = np.array([E1, E2]) + 1 - min_E
            ...
            #E_guess = np.exp(np.interp(log(P), log([P1, P2]), log([E_scaled[0], E_scaled[1]])))

            #A_guess = A1 + (P - P1) / (P2 - P1) * (A2 - A1) #np.interp(P, [P1, P2], [A1, A2])
            #b_guess = b1 + (P - P1) / (P2 - P1) * (b2 - b1) #np.interp(P, [P1, P2], [b1, b2])
            #E_guess = E1 + (P - P1) / (P2 - P1) * (E2 - E1) #np.interp(P, [P1, P2], [E1, E2])
            ...
            SMALL = 1e-300
            k1 = np.maximum(k1, SMALL)
            k2 = np.maximum(k2, SMALL)
            ...
            y = log(k)
            X = np.column_stack([np.ones_like(T), log(T), 1.0 / T])
            (c0, c1, c2), *_ = np.linalg.lstsq(X, y, rcond=None)
            A_guess = np.exp(c0)
            b_guess = c1
            E_guess = c2 * ct.gas_constant

            try:
                (A, b, E), _ = curve_fit(
                    Arrhenius_evaluate, T, k,
                    p0=[A_guess, b_guess, E_guess],
                    maxfev=10000,
                )
                if np.isnan(A) or np.isnan(b) or np.isnan(E):
                    raise RuntimeError("curve_fit returned nan")
                A, b, E = A_guess, b_guess, E_guess
            except Exception as exc:
                print(f"curve_fit failed for {reaction} ({exc}); "
                      "falling back to linear log-space least squares")
                A, b, E = A_guess, b_guess, E_guess

            print("Interpolation coefficient: ", reaction)
            print("P = ", "%.2E" % (P), ", P0 = ", "%.2E" % (P1), "P1 = ", "%.2E" % (P2))
            print("A = ", "%.2E" % (A), ", A1 = ", "%.2E" % (A1), "A2 = ", "%.2E" % (A2))
            print("b = ", "%.2E" % (b), ", b1 = ", "%.2E" % (b1), "b2 = ", "%.2E" % (b2))
            print("E = ", "%.2E" % (E), ", E1 = ", "%.2E" % (E1), "E2 = ", "%.2E" % (E2))
...
def validate_temp_point(plog_reactions, conv_reactions, temperature, pressure):
    ...
    for r1, r2 in zip(plog_reactions, conv_reactions):
        ...
        print(f'{r1.equation:<40} \t Plog rate : {logr:<20} \t Fixed P rate : {fixr:<20} \t Relative change :  {change:<20}')
